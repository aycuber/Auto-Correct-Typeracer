...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def helper(par):
        par_no_punc = remove_punctuation(par).lower().split()
        for i in subject:
            for j in par_no_punc:
                if i == j:
                    return True
        return False
    return helper
...
def accuracy(typed, source):
    ...
    if typed == '' and source == '':
        ...
        for i in range(len(typed_words)):
            if source_words[i] != typed_words[i]:
                incorrect += 1
    return ((len(typed_words) - incorrect)/len(typed_words)) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    dict = {i: diff_function(typed_word, word_list[i], limit) for i in range(len(word_list))}
    which = min(dict.items(), key = operator.itemgetter(1))[0]
    if dict[which] <= limit:
        return word_list[which]
    else:
        return typed_word
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    def helper(limit2, i):
        if limit2 >= limit:
            ...
            if source[i] == typed[i]:
                return helper(limit2, i+1)
            else:
                return helper(limit2+1, i+1) + 1
    return helper(0, 0)
    # END PROBLEM 6
...
def report_progress(typed, source, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    def helper(i):
        if i >= len(typed) or typed[int(i)] != source[int(i)]:
            return i
        else:
            return helper(i+1)
    prog = helper(0)/len(source)
...
def time_per_word(words, timestamps_per_player):
    ...
    for j in range(len(times)):
        for i in range(len(timestamps_per_player[0]) - 1):
            times[j] += [timestamps_per_player[j][i+1] - timestamps_per_player[j][i]]
...
def fastest_words(match):
    ...
    for i in range(len(get_all_times(match))):
        times += [[]]
    for i in range(len(get_all_times(match)[0])):
        for j in range(len(times)):
            temp.append(time(match, j, i))
        times[temp.index(min(temp))] += [get_word(match, i)]
        temp = []
    return times
